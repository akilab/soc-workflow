...
var actionLabel = map[string]string{
	...
	"POST /api/phases":         "段階の追加",
	"PUT /api/phases/{key}":    "段階の変更",
	"DELETE /api/phases/{key}": "段階の削除",
	"PUT /api/phases/order":    "段階の並べ替え",

	"POST /api/tasks":         "タスクの追加",
	"PUT /api/tasks/{key}":    "タスクの変更",
	"DELETE /api/tasks/{key}": "タスクの削除",
	"PUT /api/tasks/order":    "タスクの並べ替え",
