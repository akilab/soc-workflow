...
type Step struct {
	...
	// Goto は、この手順のあとに移る先のフロー。空なら移らない。
	//
	// 調べて分かった時点で別の事象になることがある（EDR の検知を追ったら
	// 暗号化が始まっていた、など）。そこでこの経路は終わり、続きは相手の
	// フローになる。行き止まりの「終了」と違い、**行き先を言う終わり**。
	//
	// 対応（Task）ではなく手順に持たせる。同じ「エスカレーションする」でも、
	// 移り先はフローごとに違うため。
