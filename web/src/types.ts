 ...
 *
 * 固定の 3 フェーズではなく設定できる並びにしてある。フローに出てくる相手は
...
/**
 * 対応のフェーズ。受信・確認、情報収集、封じ込め、復旧、記録・報告など。
 *
 * 図の列ではなく、ボックスの色とラベルで表す。フェーズを列にすると、受信の直後に
 * 報告へ飛ぶようなフローで線が端から端まで伸びて戻ってくる。フェーズは時間とともに
...
/**
 * 対応の種類。作業そのものではなく、フローの中での役割を表す。
 *
 * "close" は終了。完了させると、その経路はそこで終わる。以降の手順は
 * 対象外になる。普通の対応として「クローズ」を作ることもできるが、
...
export interface EventFlow {
  ...
  severity: Severity;
  /**
   * このフローが使う担当と、その並び。空なら全体の担当をそのまま使う。
   *
 ...
 * 1 つのフローで同じ対応を 2 回使うことがあるため。
 */
export interface Step {
  id: string;
  /** Go 側は TaskKey。json タグが task。フェーズ（色）が決まる。 */
  task: string;
  /** 担当。図のどの列に座るかが決まる。 */
  lane: string;
  title: string;
  ...
  /**
   * 元にしたフローの、どの手順から来たか。空ならこのフローで足された手順。
   *
   * 手順 ID はフローをまたいで一意なので、写すと振り直される。
